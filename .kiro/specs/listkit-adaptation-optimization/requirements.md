...
2. 高频实时页面必须保留原行为：
   ...
   - 工具面板保留 PK 状态和倒计时刷新。
3. `ListCellProvider` / `ListProviderSection` 只能作为迁移兼容层，标准页面应使用 `ListSection`、`Row(model:id:cell:)`、`.refreshID`、`.refresh(...)` 和 `.onSelect`。
...
6. Supplementary tap 安装不得移除 view 上已有的其他 tap 手势。
