...
@MainActor
final class TableListAdapterLifecycleTests: XCTestCase {
    /// 页面已释放时，新的同步及异步 apply 都应取消，且不执行内容构建闭包。
    func testApplyAfterViewReleaseCancelsWithoutBuildingContent() async {
        var view: UITableView? = makeTableView()
        let adapter = TableListAdapter<Int>(tableView: view!)
    ...
    /// 在真实 apply 的布局完成回调之前释放视图，确定性覆盖视图释放后仍推进队列的路径。
    func testLayoutCompletionCancelsQueuedMutationsAfterViewRelease() async throws {
        ...
        var view: DelayedLayoutTableView? = DelayedLayoutTableView(
        ...
        var adapter: TableListAdapter<Int>? = TableListAdapter(tableView: view!)
        weak var releasedView = view
    ...
    private func sections(value: String = "initial") -> [TableSection<Int>] {
        [TableSection(0) {
            TableRow(1, model: value, cell: UITableViewCell.self) { _, _, _ in }
        }]
    }
}
...
@MainActor
private final class DelayedLayoutTableView: UITableView {
